...
<Info>
  `Prerequisite 1` You should be all setup with your [API Credentials](/pages/get-started/credentials) before starting this tutorial.
  `Prerequisite 2` You should have successfully trained at least one [Avatar](https://app.argil.ai/avatars) from the app.
</Info>
...
<Steps>
  ...
  <Step icon="square-plus" title="Create a video">
    ...
    <Tip>
      Check the [Video creation API Reference](/pages/api-reference/endpoint/create-video) to run the request using an interactive UI.
    </Tip>
  ...
  <Step icon="badge-check" title="Check for updates until your avatar's video generation is finished">
    Get your video's updates by running a GET request on the `/videos/[video_id]` route.
    <Tip>
      Check the [Videos API Reference](/pages/api-reference/endpoint/video) to run the request using an interactive UI.
    </Tip>
    <Check>
